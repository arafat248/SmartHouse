...
import { useGetDepositReportQuery } from '../../../features/reports/reportsApi';
...
export function DepositReportTab() {
  ...
  const { data: reportData, isLoading, isError, refetch } = useGetDepositReportQuery(
      ...
      {reportData && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             <StatCard
                title="Total Filtered Deposits"
                value={`$${reportData.summary.total_deposit.toFixed(2)}`}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200">
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  {reportData.results.map((deposit) => (
                    <tr key={deposit.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">
                        {deposit.member_detail?.user?.first_name} {deposit.member_detail?.user?.last_name}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                  ...
                  {reportData.results.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-6 py-8 text-center text-sm text-slate-400">No deposits found for selected filters.</td>
                    </tr>
                  )}
                </tbody>
              </table>
